...
module.exports = function (sequelize, DataTypes) {
  const DoctorProfile = sequelize.define(
    'doctor_profile',
    {
      ...
      doctor_password: {
        type: DataTypes.STRING(256), // Increased length for bcrypt hashed password
        allowNull: true,
      },
      password_salt: {
    ...
    indexes: [
      ...
      },
      {
        ...
        unique: true,
        using: 'BTREE',
        fields: [{ name: 'doctor_email', length: 60 }],
      },
      {
        name: 'doc_mob',
        unique: true,
        using: 'BTREE',
        fields: [{ name: 'doctor_mobile', length: 15 }],
      },
      {
        name: 'doctor_profile_idx_is_dummy_doctor_id',
        using: 'BTREE',
        fields: [{ name: 'is_dummy' }, { name: 'doctor_id' }],
      },
      {
        name: 'doctor_id',
        using: 'BTREE',
    ...
    //     if (doctor.doctor_password) {
    //       const { salt, hash } = generateHash(doctor.doctor_password);
    //       doctor.password_salt = salt;
    //       doctor.doctor_password = hash;
    //     }
    //   },
    //   beforeUpdate: async (doctor) => {
    //     if (doctor.changed('doctor_password')) {
    //       const { salt, hash } = generateHash(doctor.doctor_password);
    //       doctor.password_salt = salt;
    //       doctor.doctor_password = hash;
    //     }
  ...
  // DoctorProfile.prototype.comparePassword = function (password) {
  //   const hash = crypto.pbkdf2Sync(password, this.password_salt, 10000, 64, 'sha512').toString('hex');
  //   return this.doctor_password === hash;
  // };
